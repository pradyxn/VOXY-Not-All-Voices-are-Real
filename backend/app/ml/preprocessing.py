...
def split_windows(audio: np.ndarray) -> list[np.ndarray]:
    """Return raw windows; inference applies the model's required padding."""
    if len(audio) <= TARGET_SAMPLES:
        # Do not zero-pad here. The official AASIST evaluation path repeats
        # short waveforms until the fixed model input length is reached.
        return [audio]
